...
# Do a qr transactions between the wallets on the wallet world
# Has two arguments
...
while true; do
    ...
    # Generate a new qr sender string
    echo "Transaction with $WALLET_PASSWORD and $RECIEVER_WALLET"
    $EPIC_WALLET_BINARY -c $DIR -p $WALLET_PASSWORD --usernet send -d $TRANSACTION_QR -m qr $COINS_PER_TRANSACTION > /dev/null

    # Recieve the Epics on the target wallet
    $EPIC_WALLET_BINARY -c $RANDOM_DIRECTORY -p $RECIEVER_WALLET --usernet receive -m qr -i $TRANSACTION_QR > /dev/null

    # Confirm the transaction on the sender
    $EPIC_WALLET_BINARY -c $DIR -p $WALLET_PASSWORD --usernet finalize -m qr -i response_$TRANSACTION_QR > /dev/null

    # This deletes the transaction qrs
    # At the same time, serves as a check if a transaction was processed
    eval rm $TRANSACTION_QR response_$TRANSACTION_QR
    if [ $? -eq 0 ]; then
